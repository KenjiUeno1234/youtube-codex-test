...
本システムは、テキスト原稿からYouTube解説動画用のスライドを自動生成するMVP（Minimum Viable Product）です。Markdown/HTML/CSS形式で出力し、必要に応じてPDFやHTMLに変換できます。
...
4. **スライド生成（Markdown/HTML/CSS）** (`06_render.js`) → `output/slides_src/deck.md`

### 任意の追加ステップ

- **PDF生成** (Marp CLI) → `output/slides_export/deck.pdf`
- **HTML生成** (Marp CLI) → `output/slides_export/deck.html`
