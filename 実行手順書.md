...
│   ├── theme.css                   # Marp用ライトテーマファイル
│   └── theme-dark.css              # Marp用ダークテーマファイル（HTML→PDF用）
├── src/                            # ソースコード
│   ├── 03_sections.js              # ①セクション分割
│   ├── 04_plan.js                  # ②スライド設計
│   ├── 05_tune.js                  # ③チューニング（静的チェック）
│   ├── 06_render.js                # ④Markdown生成
│   ├── 06_render_html.js           # ④HTML/CSS生成
│   ├── 07_html_to_marp.js          # ⑤HTML→Marp Markdown変換
│   └── utils.js                    # 共有ユーティリティ
└── package.json                    # npm設定
```
...
#### HTML/CSS + PDF形式で出力（推奨・完全自動化）

deck.htmlの内容をMarkdown形式に自動変換し、MarpでPDFを出力する完全自動化された手順:
...
3. プロンプト設計と自動チューニング → `output/03_slides_tuned.json`
4. HTML/CSS生成 → `output/slides_export/deck.html` + `styles.css`
5. HTMLからMarkdown変換 → `output/slides_src/deck_from_html.md`
6. MarpでPDF生成 → `output/slides_export/deck_from_html.pdf`

**メリット:**
- HTMLとPDFの両方を同時に生成
- deck.htmlと全く同じ内容のPDFが自動生成される
- 手動でのMarkdown編集が不要
- 再現性が完全に保証される
...
3. **チューニング、HTML/CSS生成、PDF生成**
   ```bash
   npm run tune && npm run render:html && npm run html-to-pdf
   ```

**生成されるファイル:**
- `output/slides_export/deck.html` - HTMLスライド
- `output/slides_export/styles.css` - CSSスタイル
- `output/slides_src/deck_from_html.md` - Marp形式Markdown
...
#### HTMLをMarpでPDF化（完全自動化）

`npm run render:html` で生成されたHTMLファイル（`deck.html`）をMarpを使ってPDFに変換する完全自動化された手順です。
...
このコマンドは以下を自動で実行します:
1. `03_slides_tuned.json`からMarp形式のMarkdownを生成 → `output/slides_src/deck_from_html.md`
2. MarpでPDFに変換 → `output/slides_export/deck_from_html.pdf`

**入力:**
- `output/03_slides_tuned.json` - チューニング済みスライド情報
...
**処理内容:**
- `07_html_to_marp.js`スクリプトが`03_slides_tuned.json`を読み取り
- 各スライドをMarp形式のMarkdownに自動変換
- テンプレート（list_toc, bullets, process等）に応じた適切なMarkdown構文を生成
- すべての見出しを`#`（h1）として出力（Marpで確実に表示されるようにするため）
- 専用のダークテーマ（`yt-mvp-dark`）を使用
- MarpでPDFに変換（ダークテーマ、ページネーション付き）

**重要な仕様:**

1. **見出しの扱い:**
   - すべてのスライドタイトルは`#`（h1）として出力されます
   - これは、Marpで`##`（h2）を使用すると、テーマによってはヘッダー領域に配置され見えなくなる問題を回避するためです
   - `#`を使用することで、タイトルが確実にスライドの本文として大きく表示されます

2. **ダークテーマの使用:**
   - 専用のダークテーマファイル（`config/theme-dark.css`）を使用
   - 背景色: `#1a1a1a`（ダークグレー）
   - 文字色: `#ffffff`（白）
   - 見出し色: `#ffffff`（白）- **重要: 見出しが確実に見えるように白色を指定**
   - アクセントカラー: `#7dd3c0`（ターコイズ）
...
3. **Markdownフロントマター:**
   ...
   ---
   ```
   - `backgroundColor`や`color`をフロントマターで指定しない（テーマで管理）
   - これにより、見出しの色がテーマCSSで適切に設定されます

**なぜ見出しが消えていた問題が発生したか:**
- 初期実装では、フロントマターで`backgroundColor: #1a1a1a`（ダークグレー）と`color: #ffffff`（白）を指定していました
- しかし、`theme.css`では`h1`の色が`color: #1a1a1a`（ダークグレー）に設定されていました
- 結果として、ダークグレーの背景にダークグレーの見出しが表示され、見えなくなっていました
- **解決策:** ダーク専用のテーマ（`theme-dark.css`）を作成し、見出しを白色（`#ffffff`）に設定しました

**メリット:**
- 完全自動化（手動編集不要）
- deck.htmlと全く同じ内容のPDFが生成される
- 見出しが確実に白色で表示される（背景色との競合を完全に解決）
- 再現性が100%保証される
- 一度のコマンドで完了

**方法2: 手動変換（カスタマイズが必要な場合）**

HTMLの内容を手動でMarkdown形式に落とし込む場合:

**手順1: HTMLをMarkdown形式に手動変換**

`deck.html` の内容を確認し、Markdown形式に落とし込みます:
...
output/01_sections.json
        ↓
  [② 04_plan.js or Claude Code 4.5] ← config/mapping.json
...
output/slides_src/deck.md
  ...
  [Marp CLI] (任意)
        ↓
output/slides_export/deck.pdf
```

### フロー2: HTML/CSS生成とPDF変換（推奨）
...
output/01_sections.json
        ↓
  [② Claude Code 4.5 Sonnet推奨] ← config/mapping.json
        ↓                           config/slide.schema.json
output/02_slides_plan.json
        ↓
  [③ 05_tune.js] ← config/slide.schema.json
...
output/03_slides_tuned.json
        ...
        ↓   [⑤ 07_html_to_marp.js]
        ↓        ↓
        ↓   output/slides_src/deck_from_html.md
        ↓        ↓
        ↓   [Marp CLI]
        ↓        ↓
        ↓   output/slides_export/deck_from_html.pdf
        ↓
  [④ 06_render_html.js]
        ↓
...
**コマンド:**
- **完全自動化:** `npm run build:html-pdf`
- **Claude Code使用:** `npm run sections` → Claude Codeに依頼 → `npm run tune && npm run render:html && npm run html-to-pdf`
